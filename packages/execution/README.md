...
- Claim Graph nodes remain owned by ClaimStore and CounterpointStore; its only current edge is the declared `Counterpoint --targets--> Claim` relation from `targetClaimId`.
- Claim Graph reads reconstruct a deterministic execution-local view; historical Claims remain nodes, while historical pre-T3 Counterpoints and explicit rebuttal links are not fabricated.
