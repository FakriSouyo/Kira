...
- UA15 Host-neutral Judge Current Release Core Extraction is implemented in the current isolated source-review worktree; it has not yet passed source review or merged.
...
- CLI still owns `/new` Session creation, configuration/provider/model rebinding, ConversationController switching, journal reconciliation and prepared-context commit, `/resume` and `/continue` argument validation and target selection, Judge Execution lifecycle, same-Execution acquisition, WorkflowRunner and trace composition, the narrow release store adapter, startup release reconciliation and historical artifact repair, journal/transcript and AgentEvent projection, provider composition, streaming presentation, and reload/suspend behavior. Current release ordering remains prepare, settle Execution, publish. Resume planning completes before acquisition; projection repair follows same-Execution acquisition and precedes WorkflowRunner.
