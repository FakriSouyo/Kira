...
| 17 | §24-A.3 Quarterly Financials & growth (v2) | v2 `/financials/quarterly/{s}/` hanya mengembalikan **satu kuartal** per panggilan (perlu `report_date` untuk kuartal lain) → di real `revenueGrowthYoy`/`netIncomeGrowthYoy` kosong (mock tetap mengisi). | Menghindari N-call per run; pertumbuhan kuartal di v2 juga tersedia via `company_report.financials.yoy_quarter_*_growth`. Kalau Rubrik perlu growth penuh di real, node selanjutnya bisa mengisi dari report (cached) — dikategorikan peningkatan, bukan blokir. |
