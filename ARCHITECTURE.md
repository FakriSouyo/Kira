...
| 19 | Phase 2 Task 1 — session helper & pemetaan read-back | `listRuns` + `getExecutionWithArtifacts` di `executionStoreSqlite.ts` memetakan ulang baris **manual inline** (`sourceType`/`JSON.parse` per kolom, lines 116–172), bukan memanggil helper existing `toEvidence(row)` (`evidenceStoreSqlite.ts:23`) yang dipakai semua method lain. Ini menduplikasi pemetaan snake→camel di luar `schema.ts` (melawan kutipan AGENTS.md "pemetaan eksplisit hanya di schema.ts"). | Fungsional benar & hijau (150/150); test tak menyalahkan. Namun duplikasi mapping menambah kode yang harus dirawat — ideal-nya `getExecutionWithArtifacts` reuse `toEvidence`/mapping store. Tetap berlaku; peningkatan ke-2 di Phase 2, bukan blokir. |
