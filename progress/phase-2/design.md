...
- **Task 3–5 (done, 177/177):** Task 3 `export.ts` (`/export <runId> [--format json|md|html] [--out path]`) + `renderExport{Json,Markdown,Html}` di `renderer.ts`; Task 4 `registry.ts` (`SKILLS`, `filterSkills`, `buildSkillPromptSections`) + re-export `packages/agent/src/index.ts`; Task 5 `replay.ts` (`loadFixture`, `saveFixture`, `compareSnapshot`) + `fixtures/bbca.jsonl` + `replay.test.ts` (3). Semua TDD, `pnpm check` 177/177 (24 files).
- **Task 6 mock smoke (done, 177/177):** `--mock-sectors --mock-llm` → `/judge BBCA` → `/export md` berisi `FINAL JUDGMENT` + Breakdown, `streamText` 3 chunk, `workflow-events` session/phase/tool/evidence. Hybrid bitdeer real tertunda `insufficient balance` (top-up needed) — re-smoke setelah top-up.
