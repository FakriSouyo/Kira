...
| 6 | Smoke Phase 2 | Hybrid `--mock-sectors` + bitdeer real `deepseek-ai/DeepSeek-V4-Flash` (`LLM_BASE_URL` `https://api-inference.bitdeer.ai/v1`) streaming + export | Manual mock: `pnpm check` 177/177 + `export.test.ts` judge→export md ok; hybrid real tertunda `insufficient balance` (2026-09-03) — re-smoke setelah top-up | — | **✓ mock done** (hybrid pending top-up) |

**Urutan eksekusi:** 1 → 2 → 3 → 4 → 5 → 6. Commit tiap task: `feat(phase-2): <task>` + `pnpm check` log.

**Status terbaru (2026-09-03):** Task 1–6 **selesai (mock) & hijau** (`pnpm check` **177/177**, 24 files; mock smoke judge→export ok). **Gate Phase 2:** base kini Task 6 mock + deviasi #17–#21, `pnpm check` hijau — hybrid real smoke tertunda top-up bitdeer.