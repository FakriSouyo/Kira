# ADDENDUM DESAIN · V1.2 — Financial Agent Harness
...
Revisi v1.2 mengunci tiga hal yang masih longgar di v1.1:

1. **Intent Router dibangun berbasis LLM sejak Minggu 1** — dibatasi ke structured intent, bukan tahap rule-based lalu migrasi
2. **Pemetaan tool eksplisit ke API Sectors**, disusun dari audit dokumentasi API langsung
3. **Batas Agent Configurator diperjelas** dengan contoh konkret yang dilarang

> Versi Draft 1.2 · September 2026 · Melengkapi Addendum v1.1
...
- 08 Pemetaan Tool: API Sectors → Agent
- 09 Revisi Roadmap 4 Minggu
- 10 Risiko Tambahan & Kesimpulan
...
| Tetap dari v1.0 | Berubah di v1.2 |
|-----------------|-----------------|
...
| Empat pilar desain (Bagian 03, v1.0) | Alokasi roadmap 4 minggu disesuaikan (Bagian 09 addendum ini) |

### Perubahan tambahan di v1.2

| Tetap dari v1.1 | Berubah di v1.2 |
|-----------------|-----------------|
| Intent Router hanya memilih dari Template Library, tidak pernah menyusun graph baru | Cara mencapainya: LLM classifier dipakai sejak Minggu 1, dibatasi ketat ke structured intent output — bukan rule-based dulu baru migrasi ke LLM di Minggu 3 (Bagian 09) |
| Agent Configurator terbatas ke pilihan yang sudah tervalidasi (Bagian 05) | Ditambah contoh eksplisit yang dilarang, supaya batas "checklist kurasi vs instruksi bebas" tidak ambigu saat ditulis ulang ke spec (Bagian 05) |
| Sectors API → Financial Researcher (given, belum dirinci) | Pemetaan tool eksplisit per agent, hasil audit langsung ke dokumentasi API, termasuk daftar yang secara sadar di-skip (Bagian 08, baru) |
...
| Level | Deskripsi | Status di v1.2 |
|-------|-----------|------------------|
| L0 | User harus tahu nama & sintaks command persis | Digantikan — command jadi opsional, bukan satu-satunya jalur |
| **L1** | NL bebas → Intent Router mengklasifikasi ke template + extract parameter | Scope v1.2 — dibangun untuk hackathon |
...
**Contoh output Intent Router (v1.2):** untuk pertanyaan *"Apakah BBCA layak dikoleksi, dengan analisis mendalam?"*, Router hanya mengeluarkan JSON terstruktur:

```json
{
  "intent": "company_judgment",
  "ticker": "BBCA",
  "depth": "deep",
  "include_debate": true
}
...
yang lalu di-mapping ke template `JUDGE_DEEP` yang sudah didefinisikan tim. LLM tidak pernah diminta menjawab "agent apa saja yang perlu dipanggil", hanya "template mana yang cocok". Pendekatan ini dipakai sejak Minggu 1 (lihat revisi roadmap, Bagian 09) — bukan tahap rule-based yang bermigrasi ke LLM di pertengahan proyek, karena pergantian pendekatan di tengah jalan berisiko: kalau versi awal ternyata rapuh, itu baru ketahuan saat waktu sudah mepet ke UI/robustness.
...
| Command | Deskripsi | Contoh trigger NL | Prioritas v1.2 |
|---------|-----------|-------------------|----------------|
...
| Bisa dikonfigurasi user | Tidak dibuka ke user (di v1.2) |
|-------------------------|-------------------------------|
...
**Contoh konkret yang TIDAK diperbolehkan** (ditambahkan di v1.2 setelah ditemukan ambigu dalam diskusi desain): UI Configurator yang menyediakan field bebas seperti *"Instructions: Analyze banking-specific indicators however you see fit..."* — itu instruksi/prompt bebas, persis yang dilarang di tabel atas. Bentuk yang benar: user memilih domain (mis. Banking) lalu mencentang skill/modul yang sudah ada implementasinya (Fundamental Analysis, Valuation, Foreign Flow), tanpa kolom teks bebas sama sekali. Kalau UI punya kolom yang menerima kalimat instruksi apa pun dari user, itu sinyal desainnya sudah keluar dari batas v1.2.
...
## Bagian 08 — Pemetaan Tool: API Sectors → Agent

v1.1 menyebut "Sectors API → Financial Researcher" sebagai given, tanpa rincian endpoint. Bagian ini menguncinya berdasarkan audit langsung ke dokumentasi API Sectors (bukan hanya nama endpoint), supaya tidak ada tool yang dicari agent tapi ternyata tidak ada, dan tidak ada endpoint yang dipakai di luar kebutuhan `/judge` & `/screen`.

| Agent | Tool (endpoint API Sectors) — status |
|------|------------------------------------------|
| **Financial Researcher** | Company Report ✅ · Quarterly Financials ✅ · Company Segments ✅ |
| **Market Researcher** | Daily Transaction ✅ · Foreign Flow by Symbol ✅ · Broker Summary Top 🟡 opsional (memperkaya "kenapa" di balik foreign flow, bukan blocker MVP) |
| **Supporting (bukan evidence utama)** | Broker Registry ✅ — tabel lookup broker_code → nama/origin, ditarik sekali & di-cache, dipakai mempercantik output Market Researcher |
| **News & Filings Researcher** | News ✅ · Filings ✅ · Suspensions 🟡 jika memang diperlukan. Catatan: tidak ada endpoint "Corporate Actions" yang terdokumentasi di grup ini — jangan dimasukkan ke spec sampai nama endpoint pastinya terverifikasi |
| **Screener** | Company Screener ✅ — pintu masuk `/screen`; Industries/Subindustries/Tags diperlakukan sebagai metadata/helper input untuk Screener, bukan tool agent tersendiri |

**Sengaja tidak dipakai di MVP** (disimpan sebagai visi produk, pola sama dengan `/investigate` & `/challenge`):

- **Top Brokers market-wide** — ranking harian se-market, tidak menempel ke satu ticker/claim → jadi ide agent baru "Broker Activity Analyst" di Agent Library
- **Seluruh endpoint SGX & KLSE** — menambah currency handling, skema Evidence Store baru, testing regulasi lain — jauh lebih berat dari sekadar opsi Configurator baru
- **Mining Extension (13 endpoint)** — kecuali skenario ticker demo memang perusahaan tambang, di mana 1–2 endpoint seperti commodity-price bisa jadi evidence tambahan untuk Sector Analyst

> **Catatan positioning:** Daily Transaction adalah data daily close, volume, dan market cap dengan window sampai 90 hari — bukan feed real-time. Jangan disebut "real-time market data" di pitch maupun UI; positioning yang akurat adalah "current/near-current research data + historical market context", supaya ekspektasi user (dan panel juri) tidak salah arah ke "terminal trading".

---

## Bagian 09 — Revisi Roadmap 4 Minggu

| Minggu | Fokus | Deliverable (perubahan dari v1.0 ditandai) |
|--------|-------|-------------------------------------------|
| 1 | Spine | Sama seperti v1.0 (Planner + DAG engine, skema Evidence Store) **+ Intent Router berbasis LLM sejak awal, dibatasi ketat ke structured intent (Bagian 03) yang di-mapping ke Template Library — bukan rule-based dulu lalu migrasi (lihat alasan di Bagian 03) + tool per agent dikunci sesuai pemetaan Bagian 08, mulai dari Financial Researcher (Company Report, Quarterly Financials)** |
| 2 | Debate + Judge + validasi | Sama seperti v1.0 (Market & News Researcher paralel, Bull+Bear+Debate 1 ronde, evidence validator wajib) **+ `/screen` mulai dibangun paralel (reuse Financial Researcher + Company Screener, tambah logic ranking sederhana)** |
| 3 | UI + robustness | Sama seperti v1.0 (live UI, caching, fallback data) **+ refinement akurasi Intent Classifier (bukan migrasi pendekatan — sudah LLM sejak Minggu 1), NL query jadi input utama di UI (bukan hanya slash command) + `/screen` selesai dengan reframing kriteria historis (Bagian 07) + GUI expand/collapse per section (Planner/Researcher/Evidence/Bull-Bear/Judge), bukan interactive execution graph engine** |
| 4 | Perluasan roadmap + polish | Sama seperti v1.0 (`/compare`, model routing 2 tier, dokumentasi) **+ Agent Configurator versi ringan (checklist domain/skill yang sudah ada, tanpa kolom instruksi bebas — lihat contoh dilarang di Bagian 05) sebagai bukti konsep; command lain tetap sebagai "next" di pitch** |

---

## Bagian 10 — Risiko Tambahan & Kesimpulan

Empat risiko baru muncul akibat perubahan di addendum ini, melengkapi tabel risiko Bagian 12 v1.0:
...
| **GUI execution graph over-scope** | Waktu Minggu 3 habis untuk graph engine interaktif alih-alih robustness agent | Batasi ke expand/collapse per section (Bagian 09, Minggu 3); klik node detail (tools/evidence/durasi) cukup sebagai panel statis, bukan graph engine general-purpose |

### Kesimpulan Revisi

Addendum ini tidak mengubah fondasi v1.0 — Evidence Store, validasi evidence, dan Judge tetap seperti semula. Yang berubah adalah bagaimana user masuk ke sistem (natural language, bukan hafalan command), bagaimana ekstensibilitas ditangani (Agent Library yang tumbuh dari tim + Agent Configurator terbatas, bukan authoring bebas per-user), dan — sejak v1.2 — bagaimana tool dipetakan ke API yang benar-benar terdokumentasi (Bagian 08), bukan diasumsikan dari nama endpoint. Dengan begitu, kata "harness" tetap punya makna teknis yang jujur — komposabilitas dari primitives yang tervalidasi — tanpa mengorbankan empat pilar yang sudah terbukti solid di v1.0.